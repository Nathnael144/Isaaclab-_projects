...
def reset_object_self_event(env):
...
def reset_all_self_event(env):
    return base_mdp.reset_scene_to_default(env, torch.arange(env.num_envs, device=env.device))
...
class ObjectTableSceneCfg(TableCylinderSceneCfg):
    ...
    # NOTE: Disabled by default for RL training since common RL stacks (e.g. RSL-RL PPO)
    # expect 1D observations. Enable cameras again for teleop/data collection as needed.
    front_camera = None
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        def __post_init__(self):
            ...
            self.concatenate_terms = True  # required for RSL-RL (expects 1D obs)
...
class PickPlaceG129DEX1BaseFixEnvCfg(ManagerBasedRLEnvCfg):
    ...
    def __post_init__(self):
        ...
        self.event_manager.register("reset_object_self", SimpleEvent(
            func=reset_object_self_event
        ))
        
        self.event_manager.register("reset_all_self", SimpleEvent(
            func=reset_all_self_event
        ))
